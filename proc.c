...
allocproc(void)
{
  ...
  while (!cas((volatile int *)&p->state, UNUSED, EMBRYO));
  popcli();
  // return 0;
...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  //np->state = RUNNABLE;
  cas((volatile int *)&np->state, EMBRYO, RUNNABLE);
...
exit(void)
{
  ...
  cas((volatile int *)&curproc->state, RUNNING, NEG_ZOMBIE);
...
wait(void)
{
  ...
  pushcli();//acquire(&ptable.lock);
  for(;;){
    cas((volatile int *)&curproc->state, RUNNING, NEG_SLEEPING);
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != curproc)
        continue;
      havekids = 1;
      if(cas((volatile int *)&p->state, ZOMBIE, NEG_UNUSED)){
        // Found one.
        pid = p->pid;
        p->pid = 0;
        p->parent = 0;
        p->name[0] = 0;
        curproc->chan = 0;
        cas((volatile int *)&curproc->state, NEG_SLEEPING, RUNNING);
        // release(&ptable.lock);
        cas((volatile int *)&p->state, NEG_UNUSED, UNUSED);
        popcli();
        return pid;
      }
    }
...
scheduler(void)
{
  ...
  for(;;){
    ...
    pushcli(); //acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(!cas((volatile int *)&p->state, RUNNABLE, RUNNING))
        continue;
      ...
      c->proc = 0;
      if(cas((volatile int *)&p->state, NEG_SLEEPING, SLEEPING)) {
        if(cas(&p->killed, 1, 0))
          p->state = RUNNABLE;
      }
      cas((volatile int *)&p->state, NEG_RUNNABLE, RUNNABLE);
      if(p->state == NEG_ZOMBIE){
          ...
          p->chan = 0;
       // freeproc(p);
        if(cas((volatile int *)&p->state, NEG_ZOMBIE, ZOMBIE))
          wakeup1(p->parent);
      }
    }
...
yield(void)
{
  ...
  cas((volatile int *)&p->state, RUNNING, NEG_RUNNABLE);
  sched();
  popcli();//release(&ptable.lock);
...
sleep(void *chan, struct spinlock *lk)
{
  ...
  cas((volatile int *)&p->state, RUNNING, NEG_SLEEPING);
...
wakeup1(void *chan)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if(p->chan == chan && (p->state == SLEEPING || p->state == NEG_SLEEPING)) {
      ...
      if(cas((volatile int *)&p->state, SLEEPING, NEG_RUNNABLE)) {
        p->chan = 0;
        if (!cas((volatile int *)&p->state, NEG_RUNNABLE, RUNNABLE))
